...
@protocol GRATrackerDelegate;
typedef struct __GRATrackerPrivate GRATrackerPrivate;
...
@interface GRATracker : NSObject {
 @private
  GRATrackerPrivate *private_;
  BOOL debug_;
  BOOL dryRun_;
  BOOL anonymizeIp_;
...
- (void)startTrackerWithAccountID:(NSString *)accountID
                   dispatchPeriod:(NSInteger)dispatchPeriod
                         delegate:(id<GRATrackerDelegate>)delegate;
...
// all GRATracker activity until it is done.
- (BOOL)dispatchSynchronous:(NSTimeInterval)timeout;

@end

@protocol GRATrackerDelegate <NSObject>
...
- (void)trackerDispatchDidComplete:(GRATracker *)tracker
